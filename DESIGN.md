...
**One-line definition**: an **embeddable, transport-agnostic** sync library that replicates writer-owned append-only logs peer-to-peer with cursor-based resumption, materializing them into derived SQLite views. MIT.
...
- Separate repo (`vilmire/seqscribe`), **MIT**. Candidate third submodule for the ADHDev monorepo.
