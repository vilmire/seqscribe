...
npm install && npm run check     # boundaries + typecheck + 281 tests + build + bundles
```
...
- All five §19 milestones implemented. 281 tests green across 31 files, plus two env-gated profiles (`SOAK=1`, `FRESH_SEEDS=n`).
...
- [docs/vendor-bump-v3.7.md](docs/vendor-bump-v3.7.md) — migration note for embedders moving from v3.6: the one non-compatible change (`stats()` no longer drains), the additive surface, and mixed-version rollout behavior
- Amendment rationale records, newest first: [P30–P38](docs/proposals-v3.8.md) (host-surface gaps found by reading an embedder's glue, plus repo-hygiene and wire items — all ratified into v3.7) · [P29](docs/proposals-v3.7.md) (ratified into v3.7) · [v3.5/v3.6](docs/proposals-v3.5.md) (P1–P28, integration-surfaced) · [v3.4](docs/proposals-v3.4.md) (surfaced by failing P7 runs) · [v3.3](docs/proposals-v3.3.md) (vector/harness work)
