...
>
> **Remaining (small, tracked)**: pre-cut register *requests* are not carried in snapshots (owned-request queues reset at bootstrap) · archive lives in `sq_archive` rather than the spec's `archive/<topic>.jsonl.gz` file form (adapter concern; export can drain it) · reference transport package (`seqscribe-ws`) and real-process integration remain — the library has only run against the in-memory harness.
