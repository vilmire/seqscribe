...
>
> **Fourth pass (2026-08-26)** — integration-readiness: attach now rejects `full` grants on subscribe-only topics; the better-sqlite3 adapter enforces crash-safe cross-process ownership (`BEGIN EXCLUSIVE` on `<db>.lock`); `stats()` exposes the host-guide metrics baseline; host helpers landed (`hmacAuthority`, `startFinalityLoop`, `manageReconnect`, `loadOrCreateWriterId`, `migrateLegacyJsonl`); browser storage proven — `sqliteWasmHandle` runs the full pipeline on the official sqlite-wasm build (plus a source guard: no `node:` imports in src/), **and verified in a real Chromium browser**: `npm run e2e:browser` serves a page that runs a full node on sqlite-wasm, bidirectionally full-syncs with a Node server over a real WebSocket (chain-hash equality asserted), and consumes the server's view via a Tier-2 SUB/SNAP subscription — the dashboard shape end to end; `durableObjectSqlHandle` + `beaconFetchHandler` cover the Cloudflare tier. Spec-adjacent additions recorded in [proposals-v3.5.md](proposals-v3.5.md). 125 tests (123 gating + 2 env-gated).
>
> **Remaining (small, tracked)**: **package split deferred to release, with readiness now gated** — DESIGN §8's four packages still ship as one. Moving folders while the package is unpublished would break the embedder's `file:` vendor path for no benefit, but the property the split depends on is enforced from today: `npm run check:boundaries` fails if `ws.ts` or `adapters.ts` reaches past the public type surface into core internals (they satisfied it by good layering, not by anything checking). Note the split is narrower than it reads: `seqscribe-beacon` is the reference *server*; the beacon **client** (`BeaconHub`) is core by design (DESIGN §5.7, `node.ts` owns one) and is not moving. · **browser worker/OPFS reference lives in `e2e-browser/` deliberately** — it is an executable reference *and* the browser acceptance test, and splitting those into a published package plus a separate fixture would duplicate the thing that makes it trustworthy (it is the same code the CI-adjacent preview runs). `npm run e2e:browser` and `npm run e2e:dashboard` serve it; both bundles are now built by `npm run check` via `build:e2e`, so a broken `.src.js` fails the gate instead of silently leaving a stale bundle on disk. · **production integration** (the next phase — the embedder vendors this library, so the v3.7 surface lands there on the next vendor bump; note P31 makes `stats()` non-draining, which that bump must account for — the migration is written up in [vendor-bump-v3.7.md](vendor-bump-v3.7.md)).
>
> **Closed in v3.7**: the two long-standing SPEC-vs-code divergences are resolved, both in the SPEC's favor being wrong rather than the code's — §7.6's `archive/<topic>.jsonl.gz` named a filesystem path two of the three shipped storage adapters cannot write (P35: §7.6 now specifies archive *behavior*, with `sq_archive` named as the reference choice), and `RegisterSnapshotState`'s missing `requests` field was a real decision recorded nowhere (P36: now stated in §11.6 with its remedy). Neither changed code.
...
| `messages` | §5.4 | wire message types + parse/serialize with structural validation (split from `codec`, which keeps LogEntry validation) |
| `archive` | §7.6 | cold archiving: consumer-gated batched deletion, `consumer_abandoned`, base-checkpoint/`regbase` rebuild inputs |
| `adapters` | §14 `SqliteHandle` | better-sqlite3 handle (ownership lock), `sqliteWasmHandle` (browser), `durableObjectSqlHandle` (Cloudflare) |
| `host` | host-guide | optional host-contract sugar: `hmacAuthority`, `startFinalityLoop`, `manageReconnect`, `loadOrCreateWriterId`, `migrateLegacyJsonl` (`beaconFetchHandler` lives in `beacon`) |
| `ws` | DESIGN §8 | reference socket transport: wraps an already-connected WebSocket into the §14 `Channel` |
