...
| `node.scanEntries(topic, opts)` / `node.headOrder(topic)` | bounded read-only scans: canonical order with a pinned `through` interval, or per-writer seq ranges spanning the cold archive and a ring topic's live memory tail; never creates a cursor (v3.5-P21, P25) |
| `node.subscribe(peer, {view: "tail", params: {topic}})` | live SUB delivery for a `subscribe-only` topic — `ring` (in-memory tail) **or now `full`** (durable `sq_log` tail, uncertified-consumer-friendly), identical wire shape either way (§4.5) |
| `node.pruneTopic(topic, {olderThanMs?, keepNewest?})` | local, uncertified row-count/age cap for a `full`-retention `subscribe-only` topic — the alternative to `ArchiveHub`'s cert-gated compaction when no fleet authority certifies the topic; never prunes below a registered consumer's cursor or while a `"tail"` subscriber is attached (§4.5) |
...
## 1.5 Host ergonomics added by v3.5 P10–P16 (verified under a production fleet)
...
live mixed-topic dashboard — full-synced `mesh.events` with a LOCAL ledger
...
## 4.5 Subscribe-only topic housekeeping — tail delivery + local prune (G2b)

Two extension operations for a `subscribe-only` topic whose retention is `full` — e.g. an append-only per-session transcript a dashboard tails live and expects to survive a restart. Neither is SPEC-normative (like `retireTopic`/`gcWriters` above them on `SeqscribeNodeExt`, they are local housekeeping: no signed authority, no cross-peer canonical claim), but both share `retireTopic`'s refuse-don't-degrade discipline.

**Live delivery: the `"tail"` SUB view now serves `full` as well as `ring`.** Previously `view: "tail"` (the built-in ring-tail SUB group) threw `ERR_UNKNOWN_VIEW` for anything but `retention: {mode: "ring"}`. A `full`-retention `subscribe-only` topic subscribes the identical way — `node.subscribe(peer, {view: "tail", params: {topic}})` — and gets the identical wire shape (SNAP of the last `FULL_TAIL_DEFAULT` (500) durable rows, DELTA on each new applied entry, cursor-resume through the same in-memory delta journal, SNAP-reset on cursor-beyond-retention or process-restart epoch bump). A consumer written against a ring topic's tail view needs **zero code change** to serve a `full`-retention topic instead — only the topic's own `defineTopic` policy changes. `full-sync` topics are still refused (their live delivery is the sync engine's replication push, and a second "tail" delivery path would double-deliver); `register`-kind topics still use the separate built-in `"register"` SUB group.

Consumer resume is **immune to local pruning** (below): a resubscribe replays from the in-memory DELTA journal (`SUB_DELTA_RETAIN` = 256 entries) or a fresh SNAP of whatever the store currently holds — it never re-reads store rows a prior SNAP already delivered, so pruning old rows out of `sq_log` cannot produce a gap in what a tail subscriber has already received. A subscriber beyond the journal's retention (or a fresh one) always gets a full SNAP reset, never a silent partial replay — this is the existing `handleSub` "fresh or beyond retention or epoch mismatch → SNAP reset" path, unchanged by this addition.

**Local prune: `node.pruneTopic(topic, {olderThanMs?, keepNewest?})`.** `ArchiveHub`'s own compaction (§7.6) only runs once a finality certificate exists, and certification is a fleet-authority decision — a `subscribe-only` topic with no coordinator-side certification loop watching it (the common case for a per-session topic defined on demand, after boot) can accumulate `sq_log` rows unboundedly between certs, or forever if it is never certified at all. `pruneTopic` is the uncertified alternative: a purely local row-count/age cap the host chooses for itself, requiring no cert.

```ts
await node.pruneTopic("session.abc123.transcript", { keepNewest: 5_000 });
// or: { olderThanMs: 7 * 24 * 60 * 60 * 1000 }, or both (the tighter bound wins)
```

Refuses (`ERR_MISUSE`, promise rejects) when: the topic is `register`-kind or `full-sync`-replicated (cross-peer canonical — pruning locally would fork the mesh's view of what this node has, the same reasoning `retireTopic` states for its own refusal); the topic's retention is not `full` (nothing durable for it to shrink — a ring topic already self-bounds via its in-memory tail size); neither `olderThanMs` nor `keepNewest` is given; or the topic currently has an active `"tail"` SUB subscriber (mirrors `retireTopic`'s own `hasActiveSubscriber` precondition — wait for the subscriber to disconnect, or reduce your `keepNewest` and retry once idle). **Never prunes below any registered `onEntry` consumer's cursor** — the same `store.cursorsForTopic` source `ArchiveHub.archiveNow` reads — so a live durable consumer can never have its unread tail deleted out from under it; unlike `ArchiveHub`, `pruneTopic` does **not** drop stale/abandoned cursors as a side effect (use `pruneConsumers` explicitly for that, per P18 above, before pruning a topic whose only cursor is dead). Idempotent: pruning an already-bounded topic resolves `{prunedRows: 0}`, not an error. The writer's `contig`/`chain` head (and hence `vectors()`) is untouched — only `sq_log` rows move, so a subsequent append continues the same stream, never resets to seq 1 the way `retireTopic` (which deletes the whole `sq_writers` row) does.
...
| `sync_hot` | info | sync moved ≥ `SYNC_HOT_BYTES` within one `SYNC_HOT_WINDOW_MS` window (v3.5-P24) — normal during bulk catch-up (cold start, reconnect burst, replay); it exists so "is this CPU sync traffic?" is answerable without a profiler. Check `stats().topics[t].sync` and `stats().syncHotspots` for who/what is moving. Never throttled — sustained emission with no peer ever catching up is what warrants a look |
